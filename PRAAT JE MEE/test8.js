...
function renderCopyDrill(){
  ...
  inp.placeholder=lang==='en'?'Type exactly what you see above...':lang==='tr'?'Yukarıdakini tam olarak yaz...':lang==='ar'?'اكتب بالضبط ما تراه أعلاه...':lang==='om'?'Waan armaan olitti argitu sirriitti barreessi...':lang==='ps'?'دقیقه هغه څه ولیکه چې پورته ګورې...':lang==='zh'?'请准确输入你看到的句子...':'Schrijf exact hetzelfde over...';
  card.appendChild(inp);
  const hint=document.createElement('div'); hint.className='fill-hint'; hint.style='margin-top:6px;';
  hint.textContent=lang==='en'?'💡 Capital letter at start. End with . or ?':lang==='tr'?'💡 Başta büyük harf. Sonda . veya ? ile bitir':lang==='ar'?'💡 ابدأ بحرف كبير. وانهِ بـ . أو ؟':lang==='om'?'💡 Qubee guddaan jalqabi. Tuqaa . yookiin ? tiin xumuri':lang==='ps'?'💡 په لوی توری پیل کړه. په . یا ? پای ته ورسوه':lang==='zh'?'💡 开头用大写字母。以 . 或 ? 结尾':'💡 Begin met een hoofdletter. Eindig met . of ?';
...
function tapSortChip(btn,w){ if(btn.classList.contains('used'))return; const empty=sortSlots.find(s=>!s.classList.contains('filled')); if(!empty)return; empty.querySelector('.slot-text').textContent=w; empty.classList.add('filled'); btn.classList.add('used'); }
...
function checkSort(){ const ex=sortExercises[sortIdx]; const ans=sortSlots.map(s=>s.querySelector('.slot-text').textContent).join(' ').trim(); const ok=ans===ex.answer; sortSlots.forEach(s=>{s.classList.remove('correct-slot','wrong-slot');if(s.classList.contains('filled'))s.classList.add(ok?'correct-slot':'wrong-slot');}); const fb=document.getElementById('sort-fb'); fb.className='feedback-box show'+(ok?'':' wrong-fb'); fb.textContent=ok?t('drillCorrectMsg'):`${t('drillWrongMsg')} → ${ex.answer.replace(/\s\./g,'.').replace(/\s\?/g,'?').replace(/\s!/g,'!')}`; if(ok){ speak(ex.answer.replace(/\s\./g,'.').replace(/\s\?/g,'?').replace(/\s!/g,'!')); sortSlots.forEach(s=>s.onclick=null); sortChipBtns.forEach(b=>b.onclick=null); const chkBtn=document.querySelector('#sort-container .btn-check'); if(chkBtn){ chkBtn.textContent=t('nextSentence')+' →'; chkBtn.className='btn-main'; chkBtn.style='margin-top:10px;width:100%;'; chkBtn.onclick=nextSort; } } }
...
function placeConvLine(slot){ if(!convSelected)return; if(slot.dataset.placedSpeaker){ document.querySelectorAll('#conv-src .conv-chip').forEach(b=>{if(b.dataset.speaker===slot.dataset.placedSpeaker&&b.dataset.text===slot.dataset.placedText&&b.classList.contains('placed'))b.classList.remove('placed');}); }
  ...
  slot.classList.add('has-content'); slot.classList.remove('correct-conv'); slot.dataset.placedSpeaker=speaker; slot.dataset.placedText=text;
  convSelected.classList.remove('selected'); convSelected.classList.add('placed'); convSelected=null;
}
function checkConv(){ const set=convSets[convIdx]; const slots=document.querySelectorAll('#conv-slots .order-slot'); let correct=0; slots.forEach((slot,i)=>{if(slot.dataset.placedSpeaker===set.lines[i].speaker&&slot.dataset.placedText===set.lines[i].text){slot.classList.add('correct-conv');correct++;}}); const fb=document.getElementById('conv-fb'); fb.className='feedback-box show'+(correct===set.lines.length?'':' wrong-fb'); fb.textContent=correct===set.lines.length?t('drillCorrectMsg'):`${correct} / ${set.lines.length}`; if(correct===set.lines.length){ const chkBtn=document.querySelector('#conv-order-container .btn-check'); if(chkBtn){ chkBtn.textContent=t('nextSentence')+' →'; chkBtn.className='btn-main'; chkBtn.style='margin-top:10px;width:100%;'; chkBtn.onclick=nextConv; } slots.forEach(s=>s.onclick=null); } }
